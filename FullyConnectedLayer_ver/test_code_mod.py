...
bamfile = './test/realigned_test.bam'
...
log = pd.read_table('./test/yLog20000_test.txt')
...
#        print(pileupcolumn.pos)
    ...
    if pileupcolumn.n - noCnt != 0:
        ...
        forward_baseQ[-1] = 42.
        f_bq = np.mean(forward_baseQ[:-1])/60.0  # must be normalized!!!
    ...
    arrayt = [(float(ins) / pileupcolumn.n), (float(dels) / pileupcolumn.n),
              (float((pileupcolumn.n - (ins + dels))) / pileupcolumn.n), pileupcolumn.n/100.0, indelLen / 30.0]

    ### add mapping quality in list
    if (ins > 0):
        arrayt.append((float(mapQual[0]) / ins)/70.0)
    elif (dels > 0):
        #print(indelLen)
        arrayt.append((float(mapQual[1]) / dels)/70.0)
    else:
        arrayt.append((float(mapQual[2]) / pileupcolumn.n)/70.0)
    ...
    # append y_data here
    if log.iloc[logIndex][2] == 0 and arrayt[0] >= 0.2 and float(log.iloc[logIndex][3]) / 30.0 == arrayt[4]:
        y_data = 0
    elif log.iloc[logIndex][2] == 1 and arrayt[1] >= 0.2 and float(log.iloc[logIndex][3]) / 30.0 == arrayt[4]:
        y_data = 1
    else:
        y_data = 2
    ...
    if frontLogFlag > -1 and y_data == 2:
        for k in range(logIndex - frontLogFlag):
            if log.iloc[logIndex - k - 1][2] == 0 and arrayt[0] >= 0.2 and float(
                    log.iloc[logIndex - k - 1][3]) / 30.0 == arrayt[4]:
                y_data = 0
                break
...
#        b_bq = np.mean(forward_baseQ[1:])/42.0
        b_bq = np.mean(forward_baseQ[1:])/60.0
...
#    b_bq = np.mean(forward_baseQ[(i + 1):-1])/42.0  # the last element is 0, so have to be excluded
    b_bq = np.mean(forward_baseQ[(i + 1):-1])/60.0
